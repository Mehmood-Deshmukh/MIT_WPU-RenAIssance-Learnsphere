...
const getRoadmapData = async (topic) => {
  ...
  const getAssignmentData = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/assignment/a/${assignmentId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchReview = async (attachmentId) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/attachments/getAiReview/${attachmentId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    if (state.user.assignments.includes(assignmentId)) {
      setIsSubmitted(true);
      const attachmentId = state.user.attachments.find(
        (attachment) => attachment.assignmentId.toString() === assignmentId
      ).attachmentId;
  ...
  const saveAiReview = async (review) => {
    try {
      ...
      console.log(body);
      const response = await fetch(
        "http://localhost:3000/api/attachments/saveAiReview",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const getTextFromAttachment = async (assignmentId) => {
    try {
      const attachmentId = state.user.attachments.find(
        (attachment) => attachment.assignmentId === assignmentId
      ).attachmentId;
      const response = await fetch(
        `http://localhost:3000/api/attachments/getText/${attachmentId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const pieChartData = Object.entries(
    assignmentData.rubrick.emphasisPoints
  ).map(([key, value], index) => ({
    title: key,
    value: value,
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-8">
                  <Card className="shadow-lg border-0">
                    ...
                    {Object.entries(aiReview.criteriaScore).map(
                      ([criterion, score]) => {
                        const maxScore =
                          assignmentData.rubrick.emphasisPoints[
                            criterion.replace(" Code", " Code Design")
                          ] || 25;
                        return (
